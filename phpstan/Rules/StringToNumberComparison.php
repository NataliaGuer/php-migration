 ...
 */
class StringToNumberComparison implements \PHPStan\Rules\Rule
{
    private bool $treatMixedAsPossibleString;

    public function __construct(bool $treatMixedAsPossibleString = false)
    {
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        foreach (TypeUtils::flattenTypes($string_operand) as $possibly_string) {
            if ($possibly_string instanceof ConstantStringType) {
                if (preg_match('#\D#', $possibly_string->getValue()[0] ?? '')) {
